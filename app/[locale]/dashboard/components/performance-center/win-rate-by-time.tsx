...
import { getWinRateByHour } from "./compute-metrics";

const fmt = (v: number) =>
  v.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 });

export function WinRateByTime({ trades }: { trades: FormattedTrade[] }) {
  const data = React.useMemo(() => getWinRateByHour(trades), [trades]);
  ...
  return (
    <Card className="h-72">
      <CardHeader className="p-4 pb-2 border-b h-14">
        <CardTitle className="text-base">Win Rate by Hour</CardTitle>
      </CardHeader>
      <CardContent className="p-2 h-[calc(100%-3.5rem)]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ left: 0, right: 4, top: 4, bottom: 20 }}>
            ...
            <Bar dataKey="winRate" radius={[3, 3, 0, 0]} maxBarSize={30} isAnimationActive={false}>
              {data.map((entry, i) => (
                <Cell
                  key={i}
