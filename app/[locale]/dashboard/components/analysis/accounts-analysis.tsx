...
import { AccountAnalysisSchema, AccountMetricsSchema } from "@/app/api/ai/analysis/accounts/get-account-performance"
...
export function AccountsAnalysis({ onStatusChange }: AccountsAnalysisProps) {
  ...
  const accountPerformanceToolCall = messages
    ...
    .find((part) => part.type === 'tool-getAccountPerformance') as getAccountPerformanceToolUIPart | undefined

  const accountPerformanceData = accountPerformanceToolCall?.output
    ...
    return (
      <div className="space-y-4">
        ...
        {toolCalls.map((toolCall, index) => (
          <Tool key={`${toolCall.type}-${index}`} defaultOpen={toolCall.state === 'output-available'}>
            <ToolHeader
              type={toolCall.type}
              state={toolCall.state}
              title={toolCall.type.replace('tool-', '').replace(/([A-Z])/g, ' $1').trim()}
            />
            <ToolContent>
    ...
    <Card className="relative">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>{t('analysis.accounts.description')}</CardDescription>
          </div>
          <div className="flex items-center gap-2">
            {status === 'streaming' ? (
              <div className="flex items-center gap-2">
                <RefreshCw className="h-4 w-4 animate-spin" />
                <span className="text-sm text-muted-foreground">{t('analysis.loading')}</span>
              </div>
            ) : (
              <Button
                onClick={() => sendMessage(
      ...
      <CardContent className="space-y-6">
        ...
        {accountPerformanceData && (
          <div className="space-y-4">
            <h4 className="font-medium text-sm text-muted-foreground">{t('analysis.accountPerformance')}</h4>
              ...
              <Card className="p-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-primary">
                    ${accountPerformanceData.totalPortfolioValue?.toLocaleString() || 0}
                  </div>
                  <p className="text-sm text-muted-foreground">{t('analysis.totalPortfolioValue')}</p>
                </div>
              </Card>
              <Card className="p-4">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">{t('analysis.totalAccounts')}</p>
                </div>
              </Card>
              <Card className="p-4">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">{t('analysis.portfolioRisk')}</p>
                </div>
              </Card>
            </div>

            {/* Account Performance Table */}
            {accountPerformanceData.accounts && accountPerformanceData.accounts.length > 0 && (
              <Card className="p-4">
                <h5 className="font-medium mb-4">{t('analysis.accountComparison')}</h5>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b">
                        <th className="text-left p-2 font-medium">{t('analysis.account')}</th>
                        <th className="text-left p-2 font-medium">{t('analysis.netPnL')}</th>
                        <th className="text-left p-2 font-medium">{t('analysis.winRate')}</th>
                        <th className="text-left p-2 font-medium">{t('analysis.totalTrades')}</th>
                        <th className="text-left p-2 font-medium">{t('analysis.profitFactor')}</th>
                        <th className="text-left p-2 font-medium">{t('analysis.riskLevel')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {accountPerformanceData.accounts.map((account: any, index: number) => (
                        <tr key={index} className="border-b">
                          <td className="p-2 font-medium">{account.accountNumber}</td>
                          <td className={`p-2 ${account.netPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            ${account.netPnL?.toLocaleString() || 0}
                          </td>
                          <td className="p-2">{account.winRate?.toFixed(1) || 0}%</td>
                          <td className="p-2">{account.totalTrades || 0}</td>
                          <td className="p-2">{account.profitFactor?.toFixed(2) || 0}</td>
                          <td className="p-2">
                            <Badge
                              variant="outline"
                              className={
                                account.riskLevel === 'high' ? 'text-red-600 border-red-200' :
                                  account.riskLevel === 'medium' ? 'text-yellow-600 border-yellow-200' :
            ...
            {(accountPerformanceData.bestPerformingAccount || accountPerformanceData.worstPerformingAccount) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {accountPerformanceData.bestPerformingAccount && (
                  <Card className="p-4 border-green-200 bg-green-50 dark:bg-green-950/20">
                    ...
                    <div className="space-y-1 text-sm">
                      <div><strong>{t('analysis.account')}:</strong> {accountPerformanceData.bestPerformingAccount.accountNumber}</div>
                      <div><strong>{t('analysis.netPnL')}:</strong> <span className="text-green-600">${accountPerformanceData.bestPerformingAccount.netPnL?.toLocaleString() || 0}</span></div>
                      <div><strong>{t('analysis.winRate')}:</strong> {accountPerformanceData.bestPerformingAccount.winRate?.toFixed(1) || 0}%</div>
                    </div>
                  </Card>
                )}
                ...
                {accountPerformanceData.worstPerformingAccount && (
                  <Card className="p-4 border-red-200 bg-red-50 dark:bg-red-950/20">
                    ...
                    <div className="space-y-1 text-sm">
                      <div><strong>{t('analysis.account')}:</strong> {accountPerformanceData.worstPerformingAccount.accountNumber}</div>
                      <div><strong>{t('analysis.netPnL')}:</strong> <span className="text-red-600">${accountPerformanceData.worstPerformingAccount.netPnL?.toLocaleString() || 0}</span></div>
                      <div><strong>{t('analysis.winRate')}:</strong> {accountPerformanceData.worstPerformingAccount.winRate?.toFixed(1) || 0}%</div>
                    </div>
                  </Card>
                )}
              </div>
        ...
        {process.env.NODE_ENV === 'development' && messages.length > 0 && (
          <div className="space-y-4">
            <h4 className="font-medium text-sm text-muted-foreground">Debug: Raw Messages</h4>
        ...
        {analysisToolCall ? (
          <>
            ...
            {analysisToolCall.state === 'output-available' && analysisResult && (
              <>
                {/* AI Analysis Display */}
                <div className="space-y-6">
                  ...
                  {analysisResult.dataSummary && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                      <Card className="p-4">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-primary">
                            {analysisResult.dataSummary.totalAccounts}
                          </div>
                          <p className="text-sm text-muted-foreground">{t('analysis.totalAccounts')}</p>
                        </div>
                      </Card>
                      <Card className="p-4">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-primary">
                            ${analysisResult.dataSummary.totalPortfolioValue?.toLocaleString() || 0}
                          </div>
                          <p className="text-sm text-muted-foreground">{t('analysis.totalPortfolioValue')}</p>
                        </div>
                      </Card>
                      ...
                      <Card className="p-4">
                        <div className="text-center">
                          ...
                          <p className="text-sm text-muted-foreground">{t('analysis.bestAccount')}</p>
                        </div>
                      </Card>
                    </div>
                  )}
                    ...
                    <div className="space-y-6">
                      ...
                      <Card className="p-6">
                        ...
                        <p className="text-sm leading-relaxed text-muted-foreground">
                          {analysisResult.structuredAnalysis.summary}
                        </p>
                      </Card>
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <div>
                      {t('analysis.generatedAt')}: {new Date(analysisResult.generatedAt).toLocaleString()}
                    </div>
                    <div>
                      {t('analysis.model')}: GPT-4o
                    </div>
                  </div>
                </div>
              </>
