...
function CodeChangeSummary({
  ...
  return (
    <div
      className={
        compact ? "grid gap-1.5" : "grid grid-cols-1 gap-2 min-[360px]:grid-cols-2"
      }
    >
      <div
        className={`rounded-md bg-emerald-500/8 px-2.5 py-2 dark:bg-emerald-400/10 ${compact ? "flex items-center justify-between gap-3" : ""}`}
      >
        ...
        <strong
          className={`text-sm font-medium text-emerald-700 dark:text-emerald-300 ${compact ? "shrink-0" : "mt-0.5 block"}`}
        >
          {formatter.format(additions)}
        </strong>
      </div>
      <div
        className={`rounded-md bg-rose-500/8 px-2.5 py-2 dark:bg-rose-400/10 ${compact ? "flex items-center justify-between gap-3" : ""}`}
      >
        <span className="flex items-center gap-1 text-[11px] text-muted-foreground">
        ...
        <strong
          className={`text-sm font-medium text-rose-700 dark:text-rose-300 ${compact ? "shrink-0" : "mt-0.5 block"}`}
        >
