...
export default function TickDistributionChartEmbed({ trades }: { trades: { pnl: number, quantity: number, instrument?: string }[] }) {
  ...
  React.useEffect(() => {
    const fetchTickDetails = async () => {
      try {
        ...
        const tickDetailsMap = details.reduce((acc, detail) => ({
          ...acc,
  ...
  const chartData = React.useMemo(() => {
    ...
    const filteredTrades = trades.filter(trade => trade.instrument === selectedInstrument)
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    React.useEffect(() => {
      if (active && payload && payload.length) setActiveTick(payload[0].payload.ticks)
        ...
        <div className="rounded-lg border bg-background p-2 shadow-xs">
          <div className="grid gap-2">
            <div className="flex flex-col">
              ...
              <span className="font-bold text-muted-foreground">{data.ticks}</span>
            </div>
            <div className="flex flex-col">
        ...
        <div className="w-full h-full">
          {isLoading ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-muted-foreground">Loading tick details...</div>
            </div>
          ) : !selectedInstrument ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-muted-foreground">No instruments available</div>
            </div>
          ) : chartData.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-muted-foreground">No trades found for {selectedInstrument}</div>
            </div>
          ) : (
                ...
                <Bar dataKey="count" radius={[3, 3, 0, 0]} maxBarSize={40} className="transition-all duration-300 ease-in-out">
                  {chartData.map((entry) => (
                    <Cell 
                      ...
                      opacity={activeTick === entry.ticks ? 1 : activeTick === null ? 1 : 0.3}
                    />
                  ))}
                </Bar>
              </BarChart>
