...
export default function TradeDistributionChartEmbed({ trades }: { trades: { pnl: number }[] }) {
  ...
  const chartData = React.useMemo(() => {
    ...
    const nbLoss = trades.filter(t => t.pnl < 0).length
    const nbBe = trades.filter(t => t.pnl === 0).length
        ...
        <div className="rounded-lg border bg-background p-2 shadow-xs" style={{
          ...
          <div className="grid gap-2">
            <div className="flex items-center justify-between gap-4">
              <span className="text-[0.70rem] uppercase text-muted-foreground">{t('embed.tradeDistribution.tooltip.type')}</span>
              <span className="font-bold text-muted-foreground">{data.name || item?.name}</span>
            </div>
            <div className="flex items-center justify-between gap-4">
              <span className="text-[0.70rem] uppercase text-muted-foreground">{t('embed.tradeDistribution.tooltip.percentage')}</span>
              <span className="font-bold">{`${Number(item?.value ?? data.value ?? 0).toFixed(1)}%`}</span>
            </div>
            {typeof data.count === 'number' && (
              <div className="flex items-center justify-between gap-4">
                <span className="text-[0.70rem] uppercase text-muted-foreground">{t('embed.calendar.charts.trades')}</span>
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b shrink-0 p-3 sm:p-4 h-[56px]">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-1.5">
            <CardTitle className="line-clamp-1 text-base">{t('embed.tradeDistribution.title')}</CardTitle>
            <Popover>
      ...
      <CardContent className="flex-1 min-h-0 p-2 sm:p-4">
        <div className="w-full h-full">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={chartData} cx="50%" cy="45%" innerRadius="65%" outerRadius="85%" paddingAngle={2} dataKey="value" startAngle={90} endAngle={-270} stroke="hsl(var(--background))" strokeWidth={1}>
                {chartData.map((entry, idx) => (
                  <Cell key={`cell-${idx}`} fill={entry.color} className="transition-all duration-300 ease-in-out hover:opacity-80 dark:brightness-90" />
                ))}
                <Label position="center" content={(props: Props) => {
                  ...
                  return chartData.map((entry, index) => {
                    const cum = chartData.slice(0, index).reduce((a, c) => a + c.value, 0)
                    const angle = -90 + (360 * (entry.value / 100) / 2) + (360 * cum / 100)
                    const x = cx + labelRadius * Math.cos((angle * Math.PI) / 180)
                    const y = cy + labelRadius * Math.sin((angle * Math.PI) / 180)
                    return (
                      <text key={index} x={x} y={y} textAnchor="middle" dominantBaseline="middle" className="fill-muted-foreground font-medium translate-y-2" style={{ fontSize: '12px' }}>
