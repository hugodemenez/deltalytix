...
export default function WeekdayPnLChartEmbed({ trades, theme }: { trades: { pnl: number, entryDate?: string | Date }[]; theme?: EmbedThemeVars }) {
  ...
  const weekdayData = React.useMemo(() => {
    ...
    trades.forEach(t => {
      ...
      totals[dow].total += t.pnl
      totals[dow].count += 1
    })
    return days.map(day => ({ day, pnl: totals[day].count ? totals[day].total / totals[day].count : 0, tradeCount: totals[day].count }))
  }, [trades])

  const maxPnL = React.useMemo(() => Math.max(...weekdayData.map(d => d.pnl), 0), [weekdayData])
  const minPnL = React.useMemo(() => Math.min(...weekdayData.map(d => d.pnl), 0), [weekdayData])
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    React.useEffect(() => {
      if (active && payload && payload.length) setActiveDay(payload[0].payload.day)
      ...
      <CardContent className="flex-1 min-h-0 p-2 sm:p-4">
        <div className="w-full h-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={weekdayData} margin={{ left: 0, right: 8, top: 8, bottom: 24 }}>
              ...
              <Bar dataKey="pnl" radius={[3, 3, 0, 0]} maxBarSize={40} className="transition-all duration-300 ease-in-out">
                {weekdayData.map((entry) => (
                  <Cell key={`cell-${entry.day}`} fill={getColor(entry.pnl)} opacity={activeDay === entry.day ? 1 : activeDay === null ? 1 : 0.3} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
