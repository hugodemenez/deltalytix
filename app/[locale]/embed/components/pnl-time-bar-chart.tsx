...
export default function TimeOfDayPerformanceChart({ trades }: { trades: TradeLike[] }) {
  ...
  const chartData = React.useMemo(() => {
    ...
    trades.forEach((trade) => {
      ...
      hourlyTotals[hour].totalPnl += trade.pnl
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    React.useEffect(() => {
      if (active && payload && payload.length) setActiveHour(payload[0].payload.hour)
        ...
        <div className="w-full h-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ left: 0, right: 8, top: 8, bottom: 24 }}>
              ...
              <Bar dataKey="avgPnl" radius={[3, 3, 0, 0]} maxBarSize={40} className="transition-all duration-300 ease-in-out">
                {chartData.map((entry) => (
                  <Cell key={`cell-${entry.hour}`} fill={getColor(entry.tradeCount)} opacity={activeHour === entry.hour ? 1 : activeHour === null ? 1 : 0.3} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
