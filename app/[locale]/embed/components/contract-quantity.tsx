...
export default function ContractQuantityChartEmbed({
  trades,
}: {
  trades: { quantity: number; entryDate?: string | Date }[];
}) {
  const t = useI18n();
  ...
  const getColor = (count: number) =>
    `hsl(var(--chart-8) / ${Math.max(0.2, count / maxTradeCount)})`;
