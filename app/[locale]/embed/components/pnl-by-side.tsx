...
export default function PnLBySideChartEmbed({ trades }: { trades: TradeLike[] }) {
  ...
  const chartData = React.useMemo(() => {
    ...
    return [
      {
        side: 'Long',
        pnl: showAverage ? (longTrades.length ? longPnL / longTrades.length : 0) : longPnL,
      ...
      {
        side: 'Short',
        pnl: showAverage ? (shortTrades.length ? shortPnL / shortTrades.length : 0) : shortPnL,
  ...
  const maxPnL = React.useMemo(() => Math.max(...chartData.map(d => d.pnl)), [chartData])
  const minPnL = React.useMemo(() => Math.min(...chartData.map(d => d.pnl)), [chartData])
        ...
        <div className="rounded-lg border bg-background p-2 shadow-xs" style={{
          ...
          <div className="grid gap-2">
            <div className="flex flex-col">
              <span className="text-[0.70rem] uppercase text-muted-foreground">{t('embed.pnlBySide.tooltip.averageTotal')}</span>
              <span className="font-bold">{formatCurrency(data.pnl)}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[0.70rem] uppercase text-muted-foreground">{t('embed.pnlBySide.tooltip.trades')}</span>
              <span className="font-bold text-muted-foreground">{data.tradeCount}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[0.70rem] uppercase text-muted-foreground">{t('embed.pnlBySide.tooltip.winRate')}</span>
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b shrink-0 p-3 sm:p-4 h-[56px]">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-1.5">
            <CardTitle className="line-clamp-1 text-base">{t('embed.pnlBySide.title')}</CardTitle>
            <Popover>
        ...
        <div className="w-full h-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ left: 10, right: 8, top: 8, bottom: 24 }}>
              ...
              <XAxis dataKey="side" tickLine={false} axisLine={false} height={24} tickMargin={8} tick={{ fontSize: 11, fill: 'currentColor' }} />
              <YAxis tickLine={false} axisLine={false} width={60} tickMargin={4} tick={{ fontSize: 11, fill: 'currentColor' }} tickFormatter={formatCurrency} domain={[Math.min(minPnL * 1.1, 0), Math.max(maxPnL * 1.1, 0)]} />
