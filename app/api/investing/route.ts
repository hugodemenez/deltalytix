...
export async function GET(request: Request) {
  try {
    // Get lang from URL search params, default to 'fr'
    ...
    if (events.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No events found in the Investing.com calendar',
      }, { status: 404 })
    }
    ...
    return NextResponse.json(
      { error: 'Failed to process Investing.com calendar events', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
} 