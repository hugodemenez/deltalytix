...
function ResponsiveMenuBar({
  ...
  const editorState = useEditorState({
    ...
    selector: ctx => {
      return {
        ...
        canHeading1: ctx.editor.can().toggleNode('heading', 'paragraph', { level: 1 }) ?? false,
        canHeading2: ctx.editor.can().toggleNode('heading', 'paragraph', { level: 2 }) ?? false,
        canHeading3: ctx.editor.can().toggleNode('heading', 'paragraph', { level: 3 }) ?? false,
        // Lists and blocks
        isBulletList: ctx.editor.isActive('bulletList') ?? false,
        isOrderedList: ctx.editor.isActive('orderedList') ?? false,
        isBlockquote: ctx.editor.isActive('blockquote') ?? false,
        canBulletList: ctx.editor.can().toggleList('bulletList', 'listItem') ?? false,
  ...
  useEffect(() => {
    const computeLayout = () => {
      ...
      const itemElements = Array.from(measureRef.current.children) as HTMLElement[]
      if (itemElements.length === 0) return

      // Total width with 4px gaps
      const totalWidth = itemElements.reduce((acc, el, idx) => acc + el.offsetWidth + (idx < itemElements.length - 1 ? 4 : 0), 0)
  ...
  }, [editorState.isBold, editorState.isItalic, editorState.isUnderline, editorState.isStrike, editorState.isHighlight, editorState.isHeading1, editorState.isHeading2, editorState.isHeading3, editorState.isBulletList, editorState.isOrderedList, editorState.isBlockquote])
  ...
  const allMenuItems = [
    // Text Formatting
    { type: 'button' as const, id: 'bold', icon: Bold, title: 'Bold', action: () => editor.chain().focus().toggleBold().run(), active: editorState.isBold, disabled: !editorState.canBold },
    { type: 'button' as const, id: 'italic', icon: Italic, title: 'Italic', action: () => editor.chain().focus().toggleItalic().run(), active: editorState.isItalic, disabled: !editorState.canItalic },
    { type: 'button' as const, id: 'underline', icon: UnderlineIcon, title: 'Underline', action: () => editor.chain().focus().toggleUnderline().run(), active: editorState.isUnderline, disabled: !editorState.canUnderline },
    { type: 'button' as const, id: 'strike', icon: Strikethrough, title: 'Strikethrough', action: () => editor.chain().focus().toggleStrike().run(), active: editorState.isStrike, disabled: !editorState.canStrike },
    { type: 'button' as const, id: 'highlight', icon: Highlighter, title: 'Highlight', action: () => editor.chain().focus().toggleHighlight().run(), active: editorState.isHighlight, disabled: !editorState.canHighlight },

    // Headings
    { type: 'separator' as const, id: 'heading-sep' },
    { type: 'button' as const, id: 'h1', icon: Heading1, title: 'Heading 1', action: () => editor.chain().focus().toggleHeading({ level: 1 }).run(), active: editorState.isHeading1, disabled: !editorState.canHeading1 },
    { type: 'button' as const, id: 'h2', icon: Heading2, title: 'Heading 2', action: () => editor.chain().focus().toggleHeading({ level: 2 }).run(), active: editorState.isHeading2, disabled: !editorState.canHeading2 },
    { type: 'button' as const, id: 'h3', icon: Heading3, title: 'Heading 3', action: () => editor.chain().focus().toggleHeading({ level: 3 }).run(), active: editorState.isHeading3, disabled: !editorState.canHeading3 },

    // Lists and Blocks
    { type: 'separator' as const, id: 'list-sep' },
    { type: 'button' as const, id: 'bullet', icon: List, title: 'Bullet List', action: () => editor.chain().focus().toggleBulletList().run(), active: editorState.isBulletList, disabled: !editorState.canBulletList },
    { type: 'button' as const, id: 'ordered', icon: ListOrdered, title: 'Numbered List', action: () => editor.chain().focus().toggleOrderedList().run(), active: editorState.isOrderedList, disabled: !editorState.canOrderedList },
    { type: 'button' as const, id: 'quote', icon: Quote, title: 'Blockquote', action: () => editor.chain().focus().toggleBlockquote().run(), active: editorState.isBlockquote, disabled: !editorState.canBlockquote },

    // Actions
    { type: 'separator' as const, id: 'action-sep' },
    { type: 'button' as const, id: 'image', icon: ImageIcon, title: 'Upload Image', action: () => document.getElementById('image-upload')?.click(), active: false, disabled: false },
    // Special AI dropdown trigger (rendered custom below)
    { type: 'button' as const, id: 'ai', icon: status === 'streaming' ? Loader2 : Sparkles, title: t('editor.ai.button'), action: () => { }, active: false, disabled: false },

    // History
    { type: 'separator' as const, id: 'history-sep' },
    { type: 'button' as const, id: 'undo', icon: () => <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>, title: 'Undo', action: () => editor.chain().focus().undo().run(), active: false, disabled: !editorState.canUndo },
    { type: 'button' as const, id: 'redo', icon: () => <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 10H11a8 8 0 00-8 8v2m18-10l-6 6m6-6l-6-6" /></svg>, title: 'Redo', action: () => editor.chain().focus().redo().run(), active: false, disabled: !editorState.canRedo },
  ]

  const visibleItemsList = showDropdown ? allMenuItems.slice(0, visibleItems) : allMenuItems
  const overflowItems = showDropdown ? allMenuItems.slice(visibleItems) : []

  return (
    <div ref={containerRef} className="relative flex items-center justify-between gap-1 p-2 bg-muted/30 border-b min-w-0 overflow-hidden">
      <input
        type="file"
        accept="image/*"
      ...
      <div ref={itemsRef} className="flex items-center gap-1 min-w-0 flex-1 overflow-hidden">
        {/* News SubMenu - always visible when events are provided */}
        {events && events.length > 0 && onNewsSelection && onEmbedNews && date && (
          <NewsSubMenu
            events={events}
        ...
        {visibleItemsList.map((item, index) => {
          if (item.type === 'separator') {
            return <div key={item.id} className="w-px h-6 bg-border mx-1 shrink-0" />
          }
            ...
            return (
              <DropdownMenu key={item.id}>
                ...
                <DropdownMenuContent align="start" className="w-56" sideOffset={4}>
                  <DropdownMenuItem onClick={() => onRunAIAction('explain')} disabled={status === 'streaming'}>
                    {t('editor.ai.actions.explain')}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onRunAIAction('improve')} disabled={status === 'streaming'}>
                    {t('editor.ai.actions.improvements')}
                  </DropdownMenuItem>
                  <div className="h-px bg-border my-1" />
                  <DropdownMenuItem onClick={() => onRunAIAction('suggest_question')} disabled={status === 'streaming'}>
                    {t('editor.ai.actions.suggestQuestion')}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
      ...
      <div aria-hidden className="absolute -left-[9999px] top-0 flex items-center gap-1" ref={measureRef}>
        {allMenuItems.map((item) => {
          if (item.type === 'separator') {
            return <div key={`m-${item.id}`} className="w-px h-6 bg-border shrink-0" />
          }
          const IconComponent = item.icon
          return (
            <div key={`m-${item.id}`} className="shrink-0 h-8 w-8 p-0 inline-flex items-center justify-center">
              {typeof IconComponent === 'function' && IconComponent.name === '' ? (
                <IconComponent />
              ) : (
                <IconComponent className="h-4 w-4" />
                ...
                if (item.id === 'ai') {
                  return (
                    <div key={item.id} className="flex flex-col gap-1 py-1">
                      <button
                        className={cn('px-2 py-1.5 text-left hover:bg-muted rounded')}
...
export function TiptapEditor({
  ...
  const [aiSuggestionPosition, setAiSuggestionPosition] = useState<number | null>(null)
  ...
  const { messages: editMessages, sendMessage: sendEditMessage, status: editStatus, setMessages: setEditEditMessages } = useChat({
    transport: new DefaultChatTransport({
      api: '/api/ai/editor',
      body: {
        locale: locale,
  ...
  const handleRunAIAction = useCallback((action: 'explain' | 'improve' | 'suggest_question') => {
    if (!editorRef.current) return
    ...
    const targetText = selectedText && selectedText.length > 0 ? selectedText : fullText
    ...
    switch (action) {
      case 'explain':
        instruction = 'Explain the following selection in simple, clear terms. Keep it concise.'
        break
      case 'improve':
        instruction = 'Identify concrete improvements to the following selection. Return a short bulleted list of 3-5 actionable suggestions.'
        break
    }
      ...
      if (assistantMessages.length > 0) {
        ...
        if (suggestionText && editor) {
          ...
          if (aiSuggestionPosition === null) {
            ...
            editor.commands.deleteRange({ from: aiSuggestionPosition, to: editor.state.doc.content.size })
            editor.commands.insertContent(suggestionHtml)
          }
        }
      }
  ...
  useEffect(() => {
    if (editMessages.length > 0) {
      const assistantMessages = editMessages.filter(message => message.role === 'assistant')
      if (assistantMessages.length > 0) {
        const lastMessage = assistantMessages[assistantMessages.length - 1]
        const textPart = lastMessage.parts?.find(part => part.type === 'text')
        const suggestionText = textPart?.text || null
        if (suggestionText && editor) {
          ...
          if (!aiEditHasInserted) {
            ...
            editor.commands.setTextSelection({ from: replaceFrom, to: replaceTo })
            editor.commands.insertContent(suggestionHtml)
  ...
  }, [editMessages, setAiSuggestion, editor, aiEditInsertPos, aiEditHasInserted, aiSuggestionPosition, aiEditEndPos])


  // Handle embedding news into the editor
  const handleEmbedNews = useCallback((newsIds: string[], action: 'add' | 'remove' = 'add') => {
    ...
    if (action === 'remove') {
      ...
      doc.descendants((node, pos) => {
        if (node.type.name === 'paragraph' && node.attrs && node.attrs['data-news-id']) {
    ...
    const newsHtml = eventsToAdd.map(event => {
      ...
      {!isFullscreen && (
        <div className="border rounded-lg overflow-hidden bg-background relative h-full flex flex-col">
          ...
          <ResponsiveMenuBar
            ...
            status={(status === 'streaming' || editStatus === 'streaming') ? 'streaming' : 'ready'}
