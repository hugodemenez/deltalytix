...
export function detectBehavioralPatterns(
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (prevLoss && sameInstrument && rapidReentry && currentQtySpike) {
      detections.push({
        ...
        score: Math.min(100, 55 + Math.round((15 - gapMinutes) * 2) + Math.round((current.quantity / avgQty) * 10)),
        title: 'Revenge trading detected',
        description:
          'A new position was opened shortly after a losing trade on the same instrument with increased size.',
        estimatedLoss: current.pnl < 0 ? Math.abs(current.pnl) : Math.abs(prev.pnl) * 0.3,
        confidence: 0.86,
        metadata: {
          gapMinutes,
    ...
    if (
      ...
      detections.push({
        ...
        description:
          'Oversized trade entered quickly and closed with a larger-than-usual loss, which often indicates chasing the move.',
    ...
    if (trade.pnl > 0) {
      ...
      if (positiveStreak >= 3 && trade.quantity >= avgQty * 1.75) {
        detections.push({
          ...
          description:
            'Trade size expanded significantly after multiple wins and resulted in a loss.',
  ...
  if (losingTrades.length >= 3) {
    const lastThreeLosses = losingTrades.slice(-3)
    const escalating =
      lastThreeLosses[1]?.quantity > lastThreeLosses[0]?.quantity &&
      lastThreeLosses[2]?.quantity > lastThreeLosses[1]?.quantity

    if (escalating) {
      detections.push({
        type: 'LOSS_CHASING',
        tradeIds: lastThreeLosses.map((t) => t.id),
        severity: 'HIGH',
        score: 88,
        title: 'Loss chasing sequence',
        description:
          'Three consecutive losing trades showed increasing position size, a classic loss-chasing pattern.',
        estimatedLoss: lastThreeLosses.reduce((sum, t) => sum + Math.abs(t.pnl), 0),
        confidence: 0.89,
        metadata: {
          quantities: lastThreeLosses.map((t) => t.quantity),
          pnls: lastThreeLosses.map((t) => t.pnl),
        },
      })
    }
  }
  ...
  const tradeCountValues = Object.values(sameDayCounts)
  const avgTradesPerDay = average(tradeCountValues) || 1
  ...
  Object.entries(sameDayCounts).forEach(([day, count]) => {
    if (count >= Math.max(8, avgTradesPerDay * 1.8)) {
      ...
      detections.push({
        ...
        description:
          'Trade count for the day was materially above baseline, which can signal reduced selectivity.',
