...
import type { PromoVariant } from "./tokens";
import {
  CHAT_START,
  CONNECTIONS_START,
  CTA_START,
  ...
  HEADLINE_START,
  LANDING_CHAT_START,
  LANDING_CONNECTIONS_START,
  LANDING_CTA_START,
  LANDING_DASHBOARD_START,
  LANDING_PROP_START,
  LANDING_STATS_START,
  PROP_START,
  STATS_START,
 ...
 */
export const PromoSfx: React.FC<{ readonly variant?: PromoVariant }> = ({
  variant = "ads",
}) => {
  if (variant === "landing") {
    return (
      <>
        <Sequence from={LANDING_STATS_START} durationInFrames={30} name="SFX stats">
          <Audio src={remotionSfx.whip} volume={0.85} />
        </Sequence>
        <Sequence from={LANDING_DASHBOARD_START} durationInFrames={30} name="SFX dashboard">
          <Audio src={remotionSfx.pageTurn} volume={0.75} />
        </Sequence>
        <Sequence
          from={LANDING_DASHBOARD_START + DASH_SCROLL_TO_CHAT}
          durationInFrames={30}
          name="SFX scroll chat"
        >
          <Audio src={remotionSfx.whoosh} volume={0.85} />
        </Sequence>
        <Sequence from={LANDING_CHAT_START} durationInFrames={30} name="SFX chat">
          <Audio src={kenneySfx.confirmation} volume={0.85} />
        </Sequence>
        <Sequence
          from={LANDING_DASHBOARD_START + DASH_SCROLL_TO_PROP}
          durationInFrames={30}
          name="SFX scroll accounts"
        >
          <Audio src={remotionSfx.whoosh} volume={0.85} />
        </Sequence>
        <Sequence from={LANDING_PROP_START} durationInFrames={30} name="SFX prop firm">
          <Audio src={kenneySfx.open} volume={0.85} />
        </Sequence>
        <Sequence
          from={LANDING_DASHBOARD_START + DASH_SCROLL_TO_CONN}
          durationInFrames={30}
          name="SFX scroll connections"
        >
          <Audio src={remotionSfx.uiSwitch} volume={0.85} />
        </Sequence>
        <Sequence from={LANDING_CONNECTIONS_START} durationInFrames={30} name="SFX connections">
          <Audio src={kenneySfx.select} volume={0.85} />
        </Sequence>
        <Sequence from={LANDING_CTA_START} durationInFrames={30} name="SFX CTA click">
