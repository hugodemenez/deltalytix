/** @type {import('../types.mjs').ChangelogMediaRecipe} */
export default {
  batch: 'pr-298',
  assets: [
    { file: 'landing-page-redesign', scene: 'landing-hero' },
    { file: 'landing-page-faq-and-pricing-polish', scene: 'landing-faq-expanded' },
    { file: 'landing-page-faq-and-pricing-polish-demo', scene: 'landing-pricing-stability' },
    { file: 'landing-performance-chart-carousel', scene: 'landing-features-carousel' },
    { file: 'landing-navbar-features-and-updates', scene: 'landing-navbar-updates' },
    { file: 'billing-payment-history-mobile-layout', scene: 'billing-mobile' },
    { file: 'calendar-grid-day-keys-timezone-fix', scene: 'calendar-widgets' },
  ],
}
